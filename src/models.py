...
class CollaborativeFilteringALS:
    ...
    def fit(self, interaction_matrix: csr_matrix) -> "CollaborativeFilteringALS":
        ...
        # Scale interactions by confidence
        ...
        for iteration in range(self.iterations):
            ...
            for u in range(n_users):
                ...
                if len(indices) == 0:
                    continue
                Pu = self.item_factors[indices]  # (k, f)
                cu = C[u].data                   # confidence weights
                Cu = np.diag(cu)
...
class NeuralMatrixFactorization(nn.Module):
    ...
    def __init__(
        ...
        n_items: int,
        mf_dim: int = 32,
        mlp_dims: list[int] = [256, 128, 64, 32],
        dropout: float = 0.2,
    ):
        super().__init__()
        # GMF embeddings
...
class TwoTowerRetrieval(nn.Module):
    ...
    def forward(
        ...
        genre_feats: torch.Tensor,
        neg_genre_feats: torch.Tensor,
    ) -> torch.Tensor:
        """
        Sampled softmax loss over one positive + k negatives.
        ...
        neg_vecs = self.encode_item(
            neg_items.view(-1), neg_genre_feats.view(-1, neg_genre_feats.shape[-1])
        ).view(neg_items.shape[0], neg_items.shape[1], -1)       # (B, K, D)
